...
function chainBuilder(overrides: Record<string, unknown> = {}) {
  const chain: Record<string, unknown> = {
    ...
    single: vi.fn().mockResolvedValue({ data: { id: 'quarter-1' }, error: null }),
  ...
  for (const key of Object.keys(chain)) {
    const val = chain[key];
    if (typeof val === 'function' && !(val as ReturnType<typeof vi.fn>).mockReturnThis) {
...
describe('POST /api/register/complete — Bugfixes', () => {
  ...
  describe('Orphan-Repair (Auth-User ohne Profil wiederverwenden)', () => {
    it('verwendet verwaisten Auth-User wieder und erstellt Profil', async () => {
      ...
      mockListUsers.mockResolvedValue({
        data: {
          users: [
            { id: 'orphan-user-1', email: 'test@example.com' },
          ],
        },
      });

      // Profil-Check: kein Profil vorhanden (orphaned)
      const profileCheck = vi.fn().mockResolvedValue({ data: null, error: null });
    ...
    it('gibt 409 zurueck wenn Auth-User UND Profil existieren', async () => {
      ...
      mockListUsers.mockResolvedValue({
        data: {
          users: [
            { id: 'existing-user-1', email: 'test@example.com' },
          ],
        },
      });
  ...
  describe('Non-Pilot-Street (Haushalt mit Fallback-Koordinaten)', () => {
    it('erstellt Haushalt mit Default-Koordinaten fuer unbekannte Strasse', async () => {
      ...
      const householdMaybeSingle = vi.fn().mockResolvedValue({ data: null, error: null });
      const profileInsert = vi.fn().mockResolvedValue({ error: null });
      const memberInsert = vi.fn().mockResolvedValue({ error: null });
      const verificationInsert = vi.fn().mockResolvedValue({ error: null });

...
describe('StreamingTextDisplay', () => {
  it('zeigt Text progressiv an', () => {
    const { rerender } = render(<StreamingTextDisplay text="Hallo" isStreaming={true} />)
  ...
  it('zeigt blinkenden Cursor waehrend Streaming', () => {
    const { container } = render(<StreamingTextDisplay text="Test" isStreaming={true} />)
    expect(container.querySelector('.animate-pulse')).toBeInTheDocument()
  })

  it('versteckt Cursor nach Streaming-Ende', () => {
    const { container } = render(<StreamingTextDisplay text="Fertig" isStreaming={false} />)
    expect(container.querySelector('.animate-pulse')).not.toBeInTheDocument()
  })

  it('rendert leeren Text ohne Fehler', () => {
    const { container } = render(<StreamingTextDisplay text="" isStreaming={true} />)
    expect(container.querySelector('.animate-pulse')).toBeInTheDocument()
  })
})

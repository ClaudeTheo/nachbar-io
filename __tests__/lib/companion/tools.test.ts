...
describe("companionTools", () => {
  ...
  it("create_help_request hat category als enum", () => {
    ...
    expect(tool).toBeDefined();
    const catProp = tool!.input_schema.properties.category as Record<
      string,
      unknown
    >;
  ...
  it("navigate_to hat route als enum mit erlaubten Routen", () => {
    ...
    expect(tool).toBeDefined();
    const routeProp = tool!.input_schema.properties.route as Record<
      string,
      unknown
    >;
  ...
  it("create_marketplace_listing hat type-enum mit offer/request/free", () => {
    const tool = companionTools.find(
      (t) => t.name === "create_marketplace_listing",
    );
    expect(tool).toBeDefined();
    const typeProp = tool!.input_schema.properties.type as Record<
      string,
      unknown
    >;

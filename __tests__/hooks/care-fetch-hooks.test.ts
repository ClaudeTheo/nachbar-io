...
describe('useAppointments', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockFetch.mockResolvedValue({ ok: true, json: () => Promise.resolve([{ id: 'apt-1', title: 'Arzt' }]) });
  });
...
describe('useHelpers', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockFetch.mockResolvedValue({ ok: true, json: () => Promise.resolve([{ id: 'h-1', role: 'neighbor' }]) });
  });
  ...
  beforeEach(() => {
    ...
    mockFetch.mockResolvedValue({
      ...
      json: () => Promise.resolve([
        { medication: { id: 'med-1', name: 'Metformin' }, scheduled_at: '2026-03-22T08:00:00Z', status: 'pending', snoozed_until: null },
      ]),
    });
  });
...
describe('useDocuments', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockLimit.mockResolvedValue({ data: [{ id: 'doc-1', type: 'care_report_daily' }], error: null });
  });
  ...
  it('fragt care_documents Tabelle ab', async () => {
    ...
    await waitFor(() => expect(mockFrom).toHaveBeenCalledWith('care_documents'));
  });

  it('setzt leeres Array bei Fehler', async () => {
    mockLimit.mockResolvedValueOnce({ data: null, error: { message: 'Fehler' } });
    const { result } = renderHook(() => useDocuments('senior-1'));
    await waitFor(() => expect(result.current.loading).toBe(false));
    expect(result.current.documents).toEqual([]);
  });

...
vi.mock("@/lib/care/constants", () => ({
  hasFeature: vi.fn((plan: string, feature: string) => {
    if (plan === "plus")
      return ["medications", "appointments", "checkin"].includes(feature);
    if (plan === "free")
      return ["checkin", "medical_emergency_sos"].includes(feature);
    return false;
  }),
}));
    ...
    from: vi.fn((table: string) => {
      ...
      if (table === "care_helpers") {
        ...
        const maybeSingleAssigned = vi.fn().mockResolvedValue({
          data: options.helperRole
            ? { assigned_seniors: options.assignedSeniors ?? [] }
            : null,
          error: null,
        });
        return {
...
describe("canAccessFeature", () => {
  ...
  it("gibt immer true fuer medical_emergency_sos zurueck", async () => {
    ...
    const result = await canAccessFeature(
      supabase as unknown as Parameters<typeof canAccessFeature>[0],
      "senior-1",
  ...
  it("gibt false fuer inaktives Abo zurueck", async () => {
    const { canAccessFeature } = await import("@/lib/care/permissions");
    const supabase = createMockSupabase({
      subscription: { plan: "plus", status: "cancelled" },
    });
    const result = await canAccessFeature(
      supabase as unknown as Parameters<typeof canAccessFeature>[0],
      "senior-1",
  ...
  it("gibt true fuer Trial-Abo zurueck", async () => {
    const { canAccessFeature } = await import("@/lib/care/permissions");
    const supabase = createMockSupabase({
      subscription: { plan: "plus", status: "trial" },
    });
    const result = await canAccessFeature(
      supabase as unknown as Parameters<typeof canAccessFeature>[0],
      "senior-1",

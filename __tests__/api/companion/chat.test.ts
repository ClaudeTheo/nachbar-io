...
vi.mock('@/lib/companion/tools', () => ({
  companionTools: [
    { name: 'get_waste_dates', description: 'Muelltermine', input_schema: { type: 'object', properties: {} } },
    { name: 'create_bulletin_post', description: 'Beitrag erstellen', input_schema: { type: 'object', properties: { title: { type: 'string' } }, required: ['title'] } },
  ],
}));

vi.mock('@/lib/companion/tool-executor', () => ({
  isWriteTool: (name: string) => ['create_bulletin_post', 'create_help_request', 'create_event', 'report_issue', 'create_marketplace_listing', 'update_help_offers', 'send_message', 'update_profile'].includes(name),
  executeCompanionTool: (...args: unknown[]) => mockExecuteCompanionTool(...args),
}));
...
describe('POST /api/companion/chat', () => {
  ...
  it('gibt Chat-Antwort mit Text zurueck', async () => {
    ...
    mockAnthropicCreate.mockResolvedValue({
      content: [{ type: 'text', text: 'Guten Tag! Wie kann ich Ihnen helfen?' }],
    });
  ...
  it('fuehrt Read-Tools sofort aus und gibt Ergebnis zurueck', async () => {
    ...
    const req = new NextRequest('http://localhost/api/companion/chat', {
      method: 'POST',
      body: JSON.stringify({ messages: [{ role: 'user', content: 'Wann ist Muellabfuhr?' }] }),
      headers: { 'Content-Type': 'application/json' },
    });
    const res = await POST(req);
    expect(res.status).toBe(200);
    ...
    expect(mockExecuteCompanionTool).toHaveBeenCalledWith('get_waste_dates', {}, 'u1');
  });
  ...
  it('fordert Bestaetigung fuer Write-Tools an (ohne Ausfuehrung)', async () => {
    ...
    mockAnthropicCreate.mockResolvedValue({
      content: [
        { type: 'text', text: 'Soll ich den Beitrag veroeffentlichen?' },
        {
          ...
          input: { title: 'Strassenfest', text: 'Am Samstag findet ein Fest statt.', category: 'event' },
        },
      ],
    });
    ...
    const req = new NextRequest('http://localhost/api/companion/chat', {
      method: 'POST',
      body: JSON.stringify({ messages: [{ role: 'user', content: 'Erstelle einen Beitrag fuer das Strassenfest' }] }),
      headers: { 'Content-Type': 'application/json' },
    });
    const res = await POST(req);
    expect(res.status).toBe(200);
  ...
  it('fuehrt bestaetigtes Write-Tool aus (confirmTool)', async () => {
    ...
    mockExecuteCompanionTool.mockResolvedValue({
      success: true,
      summary: 'Beitrag "Strassenfest" wurde auf dem Schwarzen Brett veroeffentlicht.',
    });
    mockAnthropicCreate.mockResolvedValue({
      content: [{ type: 'text', text: 'Der Beitrag wurde erfolgreich veroeffentlicht!' }],
    });
    ...
    const req = new NextRequest('http://localhost/api/companion/chat', {
      ...
      body: JSON.stringify({
        messages: [
          { role: 'user', content: 'Erstelle einen Beitrag fuer das Strassenfest' },
          { role: 'assistant', content: 'Soll ich den Beitrag veroeffentlichen?' },
          { role: 'user', content: 'Ja, bitte' },
        ],
        confirmTool: {
          tool: 'create_bulletin_post',
          params: { title: 'Strassenfest', text: 'Am Samstag findet ein Fest statt.' },
        },
      }),
      headers: { 'Content-Type': 'application/json' },
    });
  ...
  it('laedt Quartier-Kontext fuer den authentifizierten Nutzer', async () => {
    mockRequireAuth.mockResolvedValue({ supabase: {}, user: { id: 'user-42' } });
    mockAnthropicCreate.mockResolvedValue({
      content: [{ type: 'text', text: 'OK' }],
    });

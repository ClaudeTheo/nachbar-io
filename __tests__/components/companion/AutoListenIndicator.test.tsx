...
import { AutoListenIndicator } from "@/modules/voice/components/companion/AutoListenIndicator";
...
describe("AutoListenIndicator", () => {
  ...
  it("zeigt pulsierenden Ring bei Aktivitaet", () => {
    ...
    expect(
      container.querySelector('[data-testid="pulse-ring"]'),
    ).toBeInTheDocument();
  });
  ...
  it("hat mindestens 80px Hoehe (Senior-Modus)", () => {
    const { container } = render(
      <AutoListenIndicator isListening={true} audioLevel={0.3} />,
    );

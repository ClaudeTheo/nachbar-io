...
import { REWARD_TIERS, hasReward, getMarketplaceLimit } from '@/lib/referral-rewards';
...
describe('REWARD_TIERS', () => {
  ...
  it('Schwellen sind aufsteigend sortiert', () => {
    for (let i = 1; i < REWARD_TIERS.length; i++) {
      expect(REWARD_TIERS[i].threshold).toBeGreaterThan(REWARD_TIERS[i - 1].threshold);
    }
  });
});
...
describe('hasReward', () => {
  it('gibt true zurueck wenn Belohnung existiert', async () => {
    ...
    const result = await hasReward(mock as never, 'user-1', 'marketplace_extended');

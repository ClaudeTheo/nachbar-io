...
import { checkCareConsent, getConsentsForUser, hasAnyCareConsent, CONSENT_FEATURE_TO_API_ROUTES } from '@/lib/care/consent';

// Helper: Supabase-Mock erstellen mit korrekter Chainable-API
function mockSupabase(selectResult: { data: any; error: any }) {
  const terminal = {
  ...
  const chainable: any = {
    ...terminal,
    eq: vi.fn().mockReturnValue(undefined as any),
  };
  ...
  const selectFn = vi.fn().mockReturnValue({ ...selectResult, eq: vi.fn().mockReturnValue(chainable) });

...
export default function AufgabeDetail() {
  ...
  async function handleAccept() {
    ...
    const res = await fetch(`/api/youth/tasks/${id}/accept`, { method: 'POST' });
    if (res.ok) {
      const data = await res.json();
      setTask(data.task);
    }
  ...
  async function handleComplete() {
    setActionLoading(true);
    const res = await fetch(`/api/youth/tasks/${id}/complete`, { method: 'POST' });
    if (res.ok) {
  ...
  if (!task) {
    return <div className="p-6 text-center text-gray-500">Aufgabe nicht gefunden.</div>;
  }
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex flex-wrap gap-3">
        <span className="px-3 py-1 bg-gray-100 rounded-full text-sm">{task.category}</span>
        <span className="px-3 py-1 bg-green-100 text-green-700 rounded-full text-sm font-medium">
          {task.points_reward} Punkte
        </span>
        {task.estimated_minutes && (
          <span className="px-3 py-1 bg-gray-100 rounded-full text-sm">~{task.estimated_minutes} Min.</span>
        )}
        {task.risk_level === 'mittel' && (
          <span className="px-3 py-1 bg-amber-100 text-amber-700 rounded-full text-sm">Mittleres Risiko</span>
        )}
      </div>

      {/* Aktions-Buttons */}
      {task.status === 'open' && (
        <YouthGuard minLevel="erweitert" fallback={
          <p className="text-sm text-gray-500 bg-gray-50 p-3 rounded-lg">
            Um Aufgaben anzunehmen, benötigst du die Stufe &quot;Erweitert&quot;.
          </p>
        }>
          <button

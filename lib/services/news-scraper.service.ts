// Nachbar.io — News-Scraper Service
// Geschaeftslogik fuer den woechentlichen Scraper (bad-saeckingen.de)
...
function parseNewsArticles(html: string): Array<{
  ...
  const _articleRegex =
    /<div[^>]*class="[^"]*teaser[^"]*"[^>]*>([\s\S]*?)<\/div>\s*<\/div>\s*<\/div>/gi;
  ...
  const _simpleArticleRegex =
    /<h2[^>]*>\s*<a[^>]*>([\s\S]*?)<\/a>\s*<\/h2>|<h2[^>]*>([\s\S]*?)<\/h2>/gi;
  ...
  if (articles.length === 0) {
    ...
    const patternRegex =
      /([A-ZÄÖÜ][^.]{10,100}?)(?:\s+(?:Aktuelles|Bekanntgaben)[^.]*?)?\s+(?:icon\.crdate)?(\d{2}\.\d{2}\.\d{4})\s+([\s\S]{20,300}?)(?=Mehr erfahren|[A-ZÄÖÜ][^.]{10,100}?\s+(?:Aktuelles|Bekanntgaben)|\z)/gi;
    let pMatch;
...
/**
 * Neuigkeiten von bad-saeckingen.de scrapen, optional via Claude zusammenfassen
 * und neue Artikel in die DB eintragen.
 */
export async function scrapeNews(): Promise<ScrapeResult> {
  ...
  if (!response.ok) {
    throw new ServiceError(
      `Seitenabruf fehlgeschlagen: ${response.status}`,
      502,
    );
  }
...
Format: {"summary": "...", "relevance_score": 0, "category": "infrastructure|events|administration|weather|waste|other"}`,
        ...
        if (aiResponse.ok) {
          ...
          if (text) {
            ...
            relevance = Math.min(
              10,
              Math.max(0, parsed.relevance_score ?? relevance),
            );
          }
        }
      } catch {

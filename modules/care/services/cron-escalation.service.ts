...
// Nachbar.io — SOS-Eskalations-Cron: Offene SOS-Alerts stufenweise eskalieren (jede Minute)
...
export interface CronEscalationResult {
  checked: number;
  escalated: number;
  failed: number;
  timestamp: string;
...
// Alle offenen SOS-Alerts pruefen und bei Faelligkeit eskalieren
export async function runEscalationCron(
  supabase: SupabaseClient,
): Promise<CronEscalationResult> {
  ...
  const { data: openAlerts, error: alertsError } = await supabase
    ...
    .select(
      "id, senior_id, status, current_escalation_level, escalated_at, created_at, category",
    )
  ...
  if (alertsError) {
    ...
    log.done(500);
    throw new ServiceError("Alerts konnten nicht geladen werden", 500);
  }
  ...
  for (const alert of alerts) {
    ...
    const escalatedAt: string[] = Array.isArray(alert.escalated_at)
      ? alert.escalated_at
      : [];
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      ...
      if (attempt < maxRetries - 1) {
        ...
        log.error("db_update_failed_permanently", error, {
          alertId: alert.id,
          maxRetries,
        });
      }
    ...
    if (!updateSucceeded) {
      ...
      try {
        ...
        log.error("admin_alert_failed", adminNotifyError, {
          alertId: alert.id,
        });
      }
      continue;
    ...
    if (escalationMeta) {
      try {
        ...
        if (escalationMeta.role === null) {
          ...
          if (helpersError) {
            log.error("helpers_query_failed", helpersError, {
              alertId: alert.id,
              level: toLevel,
            });
            ...
            const { data: helperPhones } = await supabase
              ...
              .in(
                "id",
                helpers.map((h) => h.user_id),
              );
            const phoneMap = new Map(
              (helperPhones ?? []).map((u) => [u.id, u.phone as string | null]),
            );
        ...
        log.error("notification_failed", notifyError, {
          alertId: alert.id,
          level: toLevel,
        });
      }
  ...
  await writeCronHeartbeat(supabase, "escalation", {

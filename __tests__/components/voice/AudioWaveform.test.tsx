...
describe('AudioWaveform', () => {
  it('rendert 16 Balken', () => {
    const { container } = render(<AudioWaveform audioLevel={0.5} isActive={true} />);
  ...
  it('Container hat min-height 48px', () => {
    const { getByTestId } = render(<AudioWaveform audioLevel={0.5} isActive={true} />);
    const container = getByTestId('waveform-container');
    expect(container.style.minHeight).toBe('48px');
  });

  it('zeigt "Ich höre zu..." Text', () => {
    const { getByText } = render(<AudioWaveform audioLevel={0.5} isActive={true} />);
  ...
  it('Balken-Hoehe aendert sich mit audioLevel prop', () => {
    ...
    const { container: c1 } = render(<AudioWaveform audioLevel={0} isActive={true} />);
    const bars0 = c1.querySelectorAll('[data-testid="waveform-bar"]');
    const heights0 = Array.from(bars0).map(b => parseInt((b as HTMLElement).style.height));

    // Bei audioLevel 1 sind Balken groesser
    const { container: c2 } = render(<AudioWaveform audioLevel={1} isActive={true} />);
    const bars1 = c2.querySelectorAll('[data-testid="waveform-bar"]');
    const heights1 = Array.from(bars1).map(b => parseInt((b as HTMLElement).style.height));
  ...
  it('zeigt Fallback-Animation wenn nicht aktiv', () => {
    const { container } = render(<AudioWaveform audioLevel={0} isActive={false} />);
    const bars = container.querySelectorAll('[data-testid="waveform-bar"]');

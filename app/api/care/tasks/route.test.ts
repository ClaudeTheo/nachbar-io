...
const mockInsertResult = { data: { id: 'task-new', title: 'Neue Aufgabe', category: 'other' }, error: null };
...
const mockHouseholdSingle = vi.fn().mockResolvedValue(mockHouseholdResult);
...
describe('GET /api/care/tasks', () => {
  ...
  it('akzeptiert gueltige Kategorien', async () => {
    for (const cat of ['transport', 'shopping', 'companionship', 'garden', 'tech_help', 'pet_care', 'household', 'other']) {
      mockRequireAuth.mockResolvedValue({ supabase: mockSupabase, user: mockUser });
      mockRequireSubscription.mockResolvedValue(true);
...
describe('POST /api/care/tasks', () => {
  ...
  it('weist zu lange Beschreibung ab (> 1000 Zeichen)', async () => {
    const res = await POST(createPostRequest({ title: 'Test', description: 'B'.repeat(1001) }));
    expect(res.status).toBe(400);
    const json = await res.json();
  ...
  it('weist ungueltige Kategorie ab', async () => {
    const res = await POST(createPostRequest({ title: 'Test', category: 'flugzeug' }));
    expect(res.status).toBe(400);
    const json = await res.json();
    expect(json.error).toContain('Ungueltige Kategorie');
  });
  ...
  it('weist ungueltige Dringlichkeit ab', async () => {
    const res = await POST(createPostRequest({ title: 'Test', urgency: 'sofort_bitte' }));
    expect(res.status).toBe(400);
    const json = await res.json();
  ...
  it('akzeptiert gueltige Dringlichkeitsstufen', async () => {
    for (const urg of ['low', 'normal', 'high', 'urgent']) {
      mockRequireAuth.mockResolvedValue({ supabase: mockSupabase, user: mockUser });
      mockRequireSubscription.mockResolvedValue(true);
      const res = await POST(createPostRequest({ title: 'Testaufgabe', urgency: urg }));
  ...
  it('gibt 403 wenn User keinem Quartier zugeordnet', async () => {
    ...
    mockFrom.mockImplementationOnce((table: string) => {
      if (table === 'household_members') {
        return {
          select: () => createChainProxy({ data: null, error: { message: 'Not found' } }),
        };
      }
      return { select: () => createChainProxy({ data: null, error: null }) };
    });

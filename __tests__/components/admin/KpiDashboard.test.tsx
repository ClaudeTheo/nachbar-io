...
describe("KpiMetricCard", () => {
  ...
  it("zeigt positiven Trend-Pfeil", () => {
    const { container } = render(
      <KpiMetricCard title="WAH" value={50} previousValue={40} />,
    );
  ...
  it("zeigt negativen Trend-Pfeil", () => {
    const { container } = render(
      <KpiMetricCard title="Retention" value={60} previousValue={80} />,
    );
  ...
  it("zeigt Waehrungs-Format", () => {
    render(<KpiMetricCard title="MRR" value={89.5} format="currency" />);
    expect(screen.getByText("€ 89.50")).toBeInTheDocument();
  });

  it("zeigt Beschreibung", () => {
    render(
      <KpiMetricCard title="Test" value={1} description="Testbeschreibung" />,
    );
...
describe("KpiDashboard", () => {
  it("zeigt Hinweis ohne Snapshots", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Noch keine Analytics-Daten/),
      ).toBeInTheDocument();
    });
  });
  ...
  it("zeigt KPI-Cards mit Daten", async () => {
    ...
    expect(
      screen.getByText("WAH — Wöchentlich aktive Haushalte"),
    ).toBeInTheDocument();
      ...
      data: [
        {
          ...
          invite_sent: 10,
          invite_converted: 6,
          invite_conversion_rate: 60.0,
          posts_count: 12,
          events_count: 2,
          rsvp_count: 18,
          plus_subscribers: 3,
          heartbeat_coverage: 70.0,
          checkin_frequency: 2.0,
          escalation_count: 2,
          active_orgs: 1,
          mrr: 79.0,
        },
        {
          ...
          rsvp_count: 25,
          plus_subscribers: 5,
          heartbeat_coverage: 78.0,
          checkin_frequency: 3.0,
          escalation_count: 0,
          active_orgs: 2,
          mrr: 166.8,
        },

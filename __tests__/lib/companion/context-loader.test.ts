...
describe('loadQuarterContext', () => {
  ...
  it('gibt korrektes QuarterContext-Objekt zurueck', async () => {
    ...
    expect(ctx.events[0]).toEqual({ title: 'Fruehlingsfest', date: '2026-04-01' });
    expect(ctx.bulletinPosts).toHaveLength(1);
    expect(ctx.bulletinPosts[0]).toEqual({ title: 'Parkplatz gesperrt', category: 'board' });
  });
  ...
  it('gibt Standardwerte zurueck wenn createClient fehlschlaegt', async () => {
    (createClient as ReturnType<typeof vi.fn>).mockRejectedValue(new Error('DB-Fehler'));

...
export async function GET(request: Request) {
  ...
  if (!anthropicKey) {
    console.error(`${LOG_PREFIX} ANTHROPIC_API_KEY nicht gesetzt`);
  ...
  try {
    ...
    for (const pdfUrl of latestPdfs) {
      ...
      // 4. Issue-Eintrag anlegen (status: processing)
      ...
      try {
        ...
        // 7. Claude Haiku: Strukturierte Meldungen extrahieren
    ...
    const errorMsg = err instanceof Error ? err.message : String(err);
    console.error(`${LOG_PREFIX} Kritischer Fehler:`, errorMsg);

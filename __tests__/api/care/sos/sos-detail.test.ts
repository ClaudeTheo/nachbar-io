...
const mockAlert = {
  ...
  responses: [
    { id: 'r-1', helper_id: 'h-1', response_type: 'accepted', note: 'Bin unterwegs', helper: { display_name: 'Max', avatar_url: null } }
  ],
          ...
          eq: vi.fn().mockReturnValue({
            select: vi.fn().mockReturnValue({
              single: vi.fn().mockResolvedValue({
                data: { ...mockAlert, status: 'resolved', resolved_by: 'user-1', resolved_at: '2026-03-22T10:00:00Z' },
                error: null,
              }),
            }),
          }),
    ...
    vi.mocked(createClient).mockResolvedValueOnce({
      ...
      from: vi.fn((table: string) => {
        if (table === 'care_sos_alerts') return {
          select: vi.fn().mockReturnValue({
            eq: vi.fn().mockReturnValue({
              single: vi.fn().mockResolvedValue({ data: { senior_id: 'user-1' }, error: null }),
            }),
          }),
          update: vi.fn().mockReturnValue({
            eq: vi.fn().mockReturnValue({
              select: vi.fn().mockReturnValue({
                single: vi.fn().mockResolvedValue({
                  data: { ...mockAlert, status: 'cancelled', senior_id: 'user-1' },
                  error: null,
                }),
              }),
            }),

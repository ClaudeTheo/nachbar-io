...
function createMockSupabase(overrides: {
  ...
  const { profileError, sosCount = 0, lastCheckinMinutes, auditCount = 10, subscriptionError } = overrides;
  ...
  return {
    from: vi.fn((table: string) => {
      ...
      if (table === 'care_profiles') {
        if (profileError) {
          chain.then = (_: unknown, reject: (e: Error) => void) => { reject(new Error('DB down')); };
        } else {
        ...
        const checkinData = lastCheckinMinutes !== null && lastCheckinMinutes !== undefined
          ? { created_at: new Date(Date.now() - lastCheckinMinutes * 60000).toISOString() }
          : null;
        chain.maybeSingle = vi.fn().mockResolvedValue({ data: checkinData, error: null });
      } else if (table === 'care_audit_log') {
        chain.then = (resolve: (v: { count: number; error: null }) => void) =>
          resolve({ count: auditCount, error: null });
      } else if (table === 'care_subscriptions') {
        if (subscriptionError) {
          chain.then = (resolve: (v: { count: null; error: { message: string } }) => void) =>
...
describe('runCareHealthChecks', () => {
  ...
  it('gibt 5 Checks zurueck bei gesundem System', async () => {
    ...
    const checks = await runCareHealthChecks(supabase as any);

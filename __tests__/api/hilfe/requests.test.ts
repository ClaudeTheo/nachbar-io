...
function makeGetRequest(url = 'http://localhost/api/hilfe/requests'): NextRequest {
...
describe('/api/hilfe/requests', () => {
  ...
  describe('GET', () => {
    ...
    it('filtert nach quarter_id', async () => {
      ...
      mockSupabase.addResponse('help_requests', {
        data: [{ id: 'req-3', category: 'technik', status: 'open', quarter_id: 'q-1' }],
        error: null,
      });

      const { GET } = await import('@/app/api/hilfe/requests/route');
      const response = await GET(makeGetRequest('http://localhost/api/hilfe/requests?quarter_id=q-1'));
      expect(response.status).toBe(200);
      const body = await response.json();
      expect(Array.isArray(body)).toBe(true);

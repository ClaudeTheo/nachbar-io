    ...
    from: (table: string) => {
      if (table === 'caregiver_links') {
        return {
          select: () => ({
            eq: () => ({
              is: () => Promise.resolve({ data: [{ resident_id: 'res-1' }], error: null }),
            }),
          }),
        };
      }
      if (table === 'users') {
        return {
          select: () => ({
            in: () => Promise.resolve({ data: [{ id: 'res-1', display_name: 'Oma Helga', avatar_url: null }], error: null }),
          }),
        };
      }
      if (table === 'heartbeats') {
        return {
          select: () => ({
            eq: () => ({
              order: () => ({
                limit: () => Promise.resolve({ data: [{ created_at: twoHoursAgo }], error: null }),
              }),
            }),
          }),
        };
        ...
        return {
          select: () => ({
            eq: () => ({
              gte: () => ({
                order: () => ({
                  limit: () => Promise.resolve({ data: [{ status: 'good', created_at: `${today}T08:00:00Z` }], error: null }),
                }),
              }),
            }),
          }),
...
describe('CareStatusPage', () => {
  ...
  it('hat Video-Anruf und Nachricht Buttons', async () => {
    render(<CareStatusPage />);
    await waitFor(() => {
      expect(screen.getAllByTestId('video-call-res-1').length).toBeGreaterThanOrEqual(1);
      expect(screen.getAllByTestId('chat-res-1').length).toBeGreaterThanOrEqual(1);
    });
  });
  ...
  it('zeigt KEINE Inhalte (Datenschutz)', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.queryAllByText(/Nachricht/i).length).toBeGreaterThanOrEqual(1);

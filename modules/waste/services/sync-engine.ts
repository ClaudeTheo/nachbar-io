...
import { fetchIcsWasteDates } from "./ics-connector";
import { parseCsvWasteDates } from "./csv-connector";
import type { RawWasteDate, SyncResult } from "../types";
import type {
  WasteSourceRegistry,
  WasteCollectionArea,
...
async function syncSource(
  ...
  try {
    ...
    const hasChanges =
      datesInserted > 0 || datesUpdated > 0 || datesCancelled > 0;
...
async function syncArea(
  ...
  const icsUrl =
    area.ics_url || (source.connector_config as { url?: string }).url;
  ...
  if (source.connector_type === "ics") {
    if (!icsUrl) {
      return {
        ...
        unchanged: 0,
        cancelled: 0,
        errors: ["Keine ICS-URL konfiguriert"],
      };
    }
    ...
    const csvContent = (source.connector_config as { content?: string })
      .content;
    if (!csvContent) {
      return {
        fetched: 0,
        ...
        unchanged: 0,
        cancelled: 0,
        errors: ["Kein CSV-Inhalt konfiguriert"],
      };
    }
    ...
    return {
      ...
      unchanged: 0,
      cancelled: 0,
      errors: [`Connector-Typ '${source.connector_type}' nicht implementiert`],
    };
  }

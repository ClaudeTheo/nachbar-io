...
describe('Cron: Waste-Sync', () => {
  ...
  it('gibt Fehler-Array weiter wenn einzelne Quellen scheitern', async () => {
    mockRunWasteSync.mockResolvedValueOnce({
      synced: 1,
      results: [
        { source_slug: 'awb-waldshut', status: 'success', dates_inserted: 5, dates_updated: 0, dates_cancelled: 0 },
      ],
  ...
  it('gibt 500 wenn runWasteSync wirft', async () => {
    mockRunWasteSync.mockRejectedValueOnce(new Error('DB-Verbindung fehlgeschlagen'));
  ...
  it('ruft runWasteSync genau einmal auf', async () => {
    mockRunWasteSync.mockResolvedValueOnce({ synced: 0, results: [], errors: [] });

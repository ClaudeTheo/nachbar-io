...
test.describe('X14+X15: Rathaus Ankuendigung + Quartier-Isolation', () => {
  ...
  test('x14a: OrgAdmin erstellt Ankuendigung im Civic-Portal', async ({ orgAdminPage }) => {
    ...
    if (await newBtn.first().isVisible({ timeout: 5_000 }).catch(() => false)) {
      ...
      // Titel-Eingabefeld
      const titleInput = orgAdminPage.page.getByLabel(/titel|betreff|ueberschrift/i);
      ...
      // Inhalt-Eingabefeld
      const contentInput = orgAdminPage.page.getByLabel(/inhalt|nachricht|text|beschreibung/i);
      if (await contentInput.isVisible({ timeout: 2_000 }).catch(() => false)) {
        await contentInput.fill('Automatisch erzeugter E2E-Testinhalt fuer Quartier-Isolation.');
      }
      ...
      if (await publishBtn.isVisible({ timeout: 2_000 }).catch(() => false)) {
        ...
        await expect(
          orgAdminPage.page.getByText(/veroeffentlicht|gespeichert|gepostet|success/i),
      ...
      if (error && error !== 'no_credentials') {
        ...
        testAnnouncementId = String((data as Record<string, unknown>[])[0].id ?? '');
      }
    }
  ...
  test('x14b: Bewohner des gleichen Quartiers sieht Ankuendigung (X14)', async ({ residentPage }) => {
    ...
    // Per API pruefen: Ankuendigung muss fuer dieses Quartier sichtbar sein
    await waitForApiResult(
      residentPage.page,
      '/api/feed/announcements?order=created_at.desc&limit=5',
      (data) =>
        Array.isArray(data) &&
        data.some((item) =>
          typeof (item as Record<string, unknown>).title === 'string' &&
          ((item as Record<string, unknown>).title as string).includes('E2E-Ankuendigung'),
        ),
      {
        timeout: 15_000,
        message: 'Ankuendigung fuer eigenes Quartier nicht gefunden (X14)',
      },
    );

    // UI-Pruefung: Ankuendigung muss im Feed erscheinen
    await waitForRealtimeUI(
      residentPage.page,
      async () => {
        await expect(
          residentPage.page.getByText(new RegExp(TEST_ANNOUNCEMENT_MARKER.split('-')[0], 'i')),
        ).toBeVisible();
      },
      { timeout: 15_000 },
    );
  ...
  test('x15a: Bewohner eines anderen Quartiers sieht Ankuendigung NICHT (X15)', async ({
    ...
    try {
      ...
      const announcementEl = page.getByText(
        new RegExp(TEST_ANNOUNCEMENT_MARKER.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i'),
      );
      const isVisible = await announcementEl.isVisible({ timeout: 3_000 }).catch(() => false);

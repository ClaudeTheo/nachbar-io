...
describe('GET /api/caregiver/links', () => {
  ...
  it('gibt Links als Bewohner und Caregiver zurueck', async () => {
    const residentLinks = [
      { id: 'link-1', caregiver_id: 'cg-1', relationship_type: 'child', heartbeat_visible: true, revoked_at: null },
    ];
    const caregiverLinks = [
      { id: 'link-2', resident_id: 'sr-1', relationship_type: 'friend', heartbeat_visible: false },
    ];
...
describe('PATCH /api/caregiver/links/[id]', () => {
  ...
  it('widerruft einen Caregiver-Link', async () => {
    mockSupabase = createLinksPatchMock([
      // 1. Link finden
      { data: { id: 'link-1', resident_id: 'resident-1', caregiver_id: 'cg-1' }, error: null },
      // 2. Audit-Log
    ...
    const response = await PATCH(request as never, { params: Promise.resolve({ id: 'link-1' }) });
    const json = await response.json();
  ...
  it('gibt 404 wenn Link nicht dem Bewohner gehoert', async () => {
    ...
    const { PATCH } = await import('@/app/api/caregiver/links/[id]/route');
    const request = new Request('http://localhost/api/caregiver/links/link-99', {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ revoke: true }),
    });
    const response = await PATCH(request as never, { params: Promise.resolve({ id: 'link-99' }) });
    const json = await response.json();
  ...
  it('gibt 400 bei leerer Aenderung', async () => {
    ...
    expect(json.error).toContain('Keine Aenderungen');
  });

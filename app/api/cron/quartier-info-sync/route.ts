...
export async function GET(request: Request) {
  ...
  if (qError || !quarters?.length) {
    console.error(JSON.stringify({ requestId, event: "no_quarters", error: qError?.message }));
    return NextResponse.json({ message: "Keine aktiven Quartiere", requestId });
  }

  const results = { weather: 0, pollen: 0, errors: 0 };

  for (const quarter of quarters) {
    const lat = quarter.lat || 47.5535;
    const lon = quarter.lon || 7.9640;
    ...
    try {
      ...
      console.error(JSON.stringify({ requestId, event: "weather_error", quarter: quarter.id, error: String(err) }));
      results.errors++;
    }
    ...
    try {
      ...
      console.error(JSON.stringify({ requestId, event: "pollen_error", quarter: quarter.id, error: String(err) }));
      results.errors++;
    }
  }

  console.log(JSON.stringify({ requestId, event: "quartier_info_sync_done", ...results }));
  return NextResponse.json({ message: "Sync abgeschlossen", requestId, ...results });
}

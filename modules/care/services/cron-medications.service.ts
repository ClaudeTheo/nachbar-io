// modules/care/services/cron-medications.service.ts
...
export interface CronMedicationsResult {
  ok: true;
  reminders: number;
  missed: number;
  timestamp: string;
 ...
 */
function getTodayScheduledTimes(
...
// Hauptlogik: Alle aktiven Medikamente prüfen, Erinnerungen senden, Verpasste protokollieren
export async function runMedicationsCron(
  supabase: SupabaseClient,
): Promise<CronMedicationsResult> {
  // Alle aktiven Medikamente laden
  const { data: medications, error: medicationsError } = await supabase
    .from("care_medications")
    .select(
      "id, senior_id, name, dosage, schedule, instructions, managed_by, active, created_at, updated_at",
    )
    .eq("active", true);

  if (medicationsError) {
    console.error(
      "[care/cron/medications] Medikamenten-Abfrage fehlgeschlagen:",
      medicationsError,
    );
  ...
  const allMedications: CareMedication[] = decryptFieldsArray(
    medications ?? [],
    CARE_MEDICATIONS_ENCRYPTED_FIELDS,
  ) as CareMedication[];
  const now = new Date();
  ...
  for (const medication of allMedications) {
    ...
    for (const timeStr of scheduledTimes) {
      ...
      if (existingLog?.status === "snoozed" && existingLog.snoozed_until) {
        ...
        const snoozeElapsedMinutes =
          (now.getTime() - snoozedUntil.getTime()) / (1000 * 60);

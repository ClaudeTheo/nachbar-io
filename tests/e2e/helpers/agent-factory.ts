...
export async function loginAgent(agent: TestAgent): Promise<void> {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    if (response.ok()) {
      ...
      // Session-Cookies wurden via Set-Cookie-Headers im Playwright-Context gesetzt.
      // Jetzt muss der Supabase-Browser-Client die Session erkennen.
      // Dafuer die /login-Seite neu laden — der Middleware-Refresh propagiert
      // die Cookies, und der Browser-Client erkennt sie via document.cookie.

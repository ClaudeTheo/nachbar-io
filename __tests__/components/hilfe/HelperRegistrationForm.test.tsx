...
describe("HelperRegistrationForm", () => {
  ...
  it("zeigt 3 Checkboxen (verwandt, Haushalt, Rahmenbedingungen)", () => {
    render(<HelperRegistrationForm />);
    expect(
      screen.getAllByText(/nicht verwandt oder verschwaegert bis zum 2\. Grad/)
        .length,
  ...
  it("zeigt Schulungsnachweis nur bei NW-Auswahl", () => {
    ...
    expect(
      screen.queryByLabelText("Schulungsnachweis"),
  ...
  it("zeigt Bremen-Warnung bei HB-Auswahl", () => {
    ...
    expect(
      screen.queryByText(/Bremen nicht über den Entlastungsbetrag/),
    ).not.toBeInTheDocument();
    ...
    expect(
      screen.getByText(/Bremen nicht über den Entlastungsbetrag/),
    ).toBeInTheDocument();
  });

...
export default function PollDetailPage() {
  ...
  const timeAgo = formatDistanceToNow(new Date(poll.created_at), {
    addSuffix: true,
    locale: de,
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-border bg-white p-5 shadow-sm">
        <div className="mb-2 flex items-center gap-2">
          ...
          {poll.multiple_choice && (
            <Badge variant="outline">Mehrfachauswahl</Badge>
          )}
        </div>
          ...
          return (
            ...
            >
              ...
              <div className="relative flex items-center justify-between">
                <span
                  className={`font-medium ${isMyVote ? "text-quartier-green" : "text-anthrazit"}`}
                >
                  {isMyVote && "✓ "}
                  {opt.label}
                </span>
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        ...
        <span>
          {totalVotes} {totalVotes === 1 ? "Nachbar hat" : "Nachbarn haben"}{" "}
          abgestimmt
        </span>
      </div>

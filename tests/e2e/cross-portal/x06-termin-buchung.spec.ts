...
test.describe('X6: Bewohner bucht Termin → Arzt sieht Slot', () => {
  ...
  test('x6a: Bewohner navigiert zur Termin-Buchung', async ({ residentPage }) => {
    // Termin-Buchungs-Seite auf nachbar-io oeffnen
    await gotoCrossPortal(residentPage.page, 'http://localhost:3000/arzt/termine');
    ...
    const terminPage = residentPage.page.getByText(/termin|buchung|sprechstunde/i);
    await expect(terminPage.first()).toBeVisible({ timeout: 10_000 });

    await residentPage.page.screenshot({ path: 'test-results/cross-portal/x06a-termin-seite.png' });
  });

  test('x6b: Arzt prueft Terminliste auf nachbar-arzt', async ({ arztPage }) => {
    ...
    if (await appointmentList.isVisible({ timeout: 3_000 }).catch(() => false)) {
    ...
    await arztPage.page.screenshot({ path: 'test-results/cross-portal/x06b-arzt-termine.png' });
  });

  test('x6c: Arzt-Terminliste ist aktuell (Realtime-Check)', async ({ arztPage }) => {
    ...
    await arztPage.page.screenshot({ path: 'test-results/cross-portal/x06c-arzt-realtime.png' });
  });
});

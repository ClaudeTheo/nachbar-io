...
const mockExtractPdfUrls = vi.fn().mockReturnValue([
...
describe('Cron: Amtsblatt-Sync', () => {
  beforeEach(() => {
    ...
    mockFetch.mockImplementation((url: string) => {
      if (url.includes('bad-saeckingen.de/amtsblatt') && !url.endsWith('.pdf')) {
        // HTML-Seite
        return Promise.resolve({
          ok: true,
          text: () => Promise.resolve('<html><a href="Amtsblatt_2026_06.pdf">PDF</a></html>'),
        });
      }
      if (url.endsWith('.pdf')) {
        // PDF-Download
      ...
      if (url.includes('api.anthropic.com')) {
        ...
        return Promise.resolve({
          ...
          json: () => Promise.resolve({
            content: [{ type: 'text', text: '[{"title":"Test","body":"Body","category":"info"}]' }],
          }),
        });
      }
  ...
  it('gibt 500 wenn Amtsblatt-Seite nicht erreichbar', async () => {

...
// Tests fuer GET /api/pilot/households (nutzt getAdminSupabase + getPilotHouseholds Service)
...
describe("GET /api/pilot/households", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.resetModules();
    // PILOT_ADMIN_TOKEN muss gesetzt sein, sonst 401
    process.env.PILOT_ADMIN_TOKEN = "pilot-2026";
  });
  ...
  it("gibt 401 bei falschem Token zurueck", async () => {
    ...
    const req = new Request(
      "http://localhost/api/pilot/households?token=wrong",
    );
    const res = await GET(req as unknown as import("next/server").NextRequest);
    expect(res.status).toBe(401);
  });
  ...
  it("gibt Haushalte mit gueltigem Token zurueck", async () => {
    const mockData = [
      {
        ...
        house_number: "2",
        invite_code: "PILOT-ABCD-EFGH",
        quarter: { name: "Altstadt", slug: "altstadt", invite_prefix: "PILOT" },
      },
    ];

    // Service-Chain: from('households').select(...).eq(...).order(...).order(...)
    ...
    const { GET } = await import("@/app/api/pilot/households/route");
    const req = new Request(
      "http://localhost/api/pilot/households?token=pilot-2026",
    );
    const res = await GET(req as unknown as import("next/server").NextRequest);
  ...
  it("gibt 500 bei Datenbankfehler zurueck", async () => {
    const orderFn2 = vi
      .fn()
      .mockResolvedValue({ data: null, error: { message: "DB Fehler" } });
    const orderFn1 = vi.fn().mockReturnValue({ order: orderFn2 });
    const eqFn = vi.fn().mockReturnValue({ order: orderFn1 });
    mockSelect.mockReturnValue({ eq: eqFn });

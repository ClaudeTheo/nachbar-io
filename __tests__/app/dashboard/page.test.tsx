...
vi.mock('@/components/HeroCard', () => ({
  HeroCard: ({ children }: { children: React.ReactNode }) => <div data-testid="hero-card">{children}</div>,
}));
...
vi.mock('@/components/municipal/QuartierServicesSection', () => ({
  QuartierServicesSection: () => <div data-testid="quartier-services-section" />,
}));
...
interface CaregiverLink { caregiver_id: string }
interface CaregiverProfile { id: string; display_name: string; avatar_url: string | null }
...
function buildSupabaseChain(profile = mockProfile) {
  ...
  const mockCaregiverLinksIs = vi.fn(() => ({ limit: mockCaregiverLinksLimit }));
  ...
  const mockUsersSingle = vi.fn(() => Promise.resolve({ data: profile, error: null }));
  ...
  // Alerts-Chain
  const mockAlertsLimit = vi.fn(() => Promise.resolve({ data: [], error: null }));
  ...
  // Marketplace-Chain
  const mockMarketLimit = vi.fn(() => Promise.resolve({ data: [], error: null }));
  ...
  const mockFrom = vi.fn((table: string) => {
    ...
    if (table === 'caregiver_links') return { select: mockCaregiverLinksSelect };
  ...
  return {
    from: mockFrom,
    auth: {
      getUser: vi.fn(() => Promise.resolve({ data: { user: mockSupabaseUser }, error: null })),
    },
    channel: mockChannel,
...
describe('DashboardPage — Angehörige-Schnellzugriff', () => {
  ...
  it('zeigt Angehörige-Container wenn Angehörige vorhanden sind', async () => {
    ...
    mockCaregiverProfiles = [{ id: 'cg-1', display_name: 'Maria Müller', avatar_url: null }];
    currentSupabaseClient = buildSupabaseChain(mockProfile);
  ...
  it('zeigt Label "Angehörige:" mit echtem Umlaut ö', async () => {
    mockCaregiverLinks = [{ caregiver_id: 'cg-1' }];
    mockCaregiverProfiles = [{ id: 'cg-1', display_name: 'Maria', avatar_url: null }];
    currentSupabaseClient = buildSupabaseChain(mockProfile);
  ...
  it('verlinkt Angehörigen-Avatar zur Nachrichten-Seite', async () => {
    ...
    mockCaregiverProfiles = [{ id: 'cg-42', display_name: 'Anna Schmidt', avatar_url: null }];
    currentSupabaseClient = buildSupabaseChain(mockProfile);

    render(<DashboardPage />);
    await waitFor(() => {
      const links = screen.getAllByRole('link');
      const caregiverLink = links.find((l) => l.getAttribute('href') === '/messages/cg-42');
  ...
  it('zeigt title-Attribut mit Angehörigen-Name', async () => {
    ...
    mockCaregiverProfiles = [{ id: 'cg-7', display_name: 'Klaus Weber', avatar_url: null }];
    currentSupabaseClient = buildSupabaseChain(mockProfile);

    render(<DashboardPage />);
    await waitFor(() => {
      const caregiverLink = screen.getAllByRole('link').find(
        (l) => l.getAttribute('href') === '/messages/cg-7'
      );
  ...
  it('zeigt Fragezeichen als Fallback-Initial wenn kein Name vorhanden', async () => {
    ...
    mockCaregiverProfiles = [{ id: 'cg-empty', display_name: '', avatar_url: null }];
    currentSupabaseClient = buildSupabaseChain(mockProfile);
  ...
  it('rendert mehrere Angehörige-Avatare nebeneinander', async () => {
    ...
    await waitFor(() => {
      // Link zu Anna
      const linkAnna = screen.getAllByRole('link').find((l) => l.getAttribute('href') === '/messages/cg-a');
      expect(linkAnna).toBeDefined();
      // Link zu Bernd
      const linkBernd = screen.getAllByRole('link').find((l) => l.getAttribute('href') === '/messages/cg-b');

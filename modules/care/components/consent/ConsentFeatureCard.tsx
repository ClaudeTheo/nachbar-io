...
import { Shield, Heart, Pill, FileText, Phone } from 'lucide-react';
...
export function ConsentFeatureCard({ feature, label, description, granted, disabled, onChange }: Props) {
  const Icon = FEATURE_ICONS[feature];

  return (
    <label
      className={`flex items-center gap-4 rounded-2xl border-2 p-4 cursor-pointer transition-colors ${
        granted ? 'border-quartier-green bg-quartier-green/5' : 'border-border bg-white'
      } ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}
    >
      <Icon className={`h-8 w-8 flex-shrink-0 ${granted ? 'text-quartier-green' : 'text-muted-foreground'}`} />

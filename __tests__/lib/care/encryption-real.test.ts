...
describe('AES-256-GCM Crypto (echte Verschluesselung)', () => {
  ...
  it('Manipulation des Ciphertexts fuehrt zu Fehler (Authenticity)', async () => {
    ...
    // Manipuliere den Ciphertext (letztes Zeichen aendern)
    const manipuliert = verschluesselt.slice(0, -1) + (verschluesselt.endsWith('A') ? 'B' : 'A');
...
describe('Field-Encryption Helper (echte Verschluesselung)', () => {
  it('encryptFields verschluesselt nur benannte Felder', async () => {
    ...
    const verschluesselt = encryptFields(medikament, [...CARE_MEDICATIONS_ENCRYPTED_FIELDS]);
    ...
    const entschluesselt = decryptFields(verschluesselt, [...CARE_MEDICATIONS_ENCRYPTED_FIELDS]);
  ...
  it('isEncrypted erkennt verschluesselte Werte korrekt', async () => {
    vi.resetModules();
    const { isEncrypted, encryptField } = await import('@/lib/care/field-encryption');
  ...
  it('encryptField ist idempotent (doppelte Verschluesselung verhindert)', async () => {
    vi.resetModules();
    const { encryptField, decryptField } = await import('@/lib/care/field-encryption');
  ...
  it('decryptField gibt Klartext unveraendert zurueck (Abwaertskompatibilitaet)', async () => {
    ...
    expect(decryptField('Alter unverschluesselter Wert')).toBe('Alter unverschluesselter Wert');
  });

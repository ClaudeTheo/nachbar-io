...
import { getRequiredFlagForRoute, isHealthRoute } from "@/lib/health-feature-gate";
...
describe("getRequiredFlagForRoute", () => {
  ...
  it("liefert HEARTBEAT_ENABLED fuer /care/heartbeat und /care/checkin", () => {
    expect(getRequiredFlagForRoute("/care/heartbeat")).toBe("HEARTBEAT_ENABLED");
    expect(getRequiredFlagForRoute("/care/checkin")).toBe("HEARTBEAT_ENABLED");
  });
  it("liefert APPOINTMENTS_ENABLED fuer /care/appointments", () => {
    expect(getRequiredFlagForRoute("/care/appointments")).toBe("APPOINTMENTS_ENABLED");
  });

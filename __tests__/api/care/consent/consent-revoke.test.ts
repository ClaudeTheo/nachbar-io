...
vi.mock('@/lib/supabase/server', () => ({
  createClient: vi.fn().mockResolvedValue({
    ...
    from: vi.fn((table: string) => {
      ...
      if (table === 'care_consent_history') return {
        insert: vi.fn().mockResolvedValue({ error: null }),
      };
      if (table === 'care_checkins' || table === 'care_sos_alerts' || table === 'care_medications' || table === 'care_medication_logs' || table === 'care_profiles') return {
...
vi.mock('@/lib/care/types', () => ({
  CONSENT_FEATURES: ['sos', 'checkin', 'medications', 'care_profile', 'emergency_contacts'],
}));

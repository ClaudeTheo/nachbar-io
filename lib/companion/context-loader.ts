 ...
 */
export async function loadQuarterContext(userId: string): Promise<QuarterContext> {
  ...
  try {
    ...
    const wasteDate = (wasteRes.data ?? []).map((w: { collection_date: string; waste_type: string }) => ({
      date: w.collection_date,
      type: w.waste_type,
    }));

    const events = (eventsRes.data ?? []).map((e: { title: string; event_date: string }) => ({
      title: e.title,
      date: e.event_date,
    }));

    const bulletinPosts = (postsRes.data ?? []).map((p: { title: string; category: string }) => ({
      title: p.title,
      category: p.category,
    }));

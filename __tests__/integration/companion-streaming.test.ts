...
describe('Companion Streaming Integration', () => {
  ...
  it('Dialog-Modus Flow: Start -> Greeting -> Listen -> Transcript -> Processing -> Speaking -> Listen -> Stop', () => {
    ...
    // 1. Start
    act(() => { result.current.startDialog() })
    ...
    act(() => { result.current.handleTranscript('Wann kommt der Muell?') })
    expect(result.current.state).toBe('processing')

    // 4. Response -> Speaking
    act(() => { result.current.setResponse('Der Restmuell kommt am Montag.') })
    expect(result.current.state).toBe('speaking')

    // 5. TTS fertig -> Listening (Loop)
    act(() => { result.current.setSpeakingDone() })
    expect(result.current.state).toBe('listening')

    // 6. Stille -> silence_check
    act(() => { result.current.triggerSilenceCheck() })
  ...
  it('Dialog-Modus: Abschied beendet sofort', () => {
    ...
    // Abschied -> idle
    act(() => { result.current.handleTranscript('Tschuess, danke!') })
    expect(result.current.state).toBe('idle')
  })
  ...
  it('Dialog-Modus: Antwort waehrend silence_check bricht Timer ab', () => {
    ...
    act(() => { result.current.handleTranscript('Noch eine Frage') })
    expect(result.current.state).toBe('processing')
...
describe('Voice-Preferences Integration', () => {
  ...
  it('Meals im System-Prompt wenn vorhanden', () => {
    const ctx = {
      ...baseCtx,
      meals: [{ title: 'Gulaschsuppe', type: 'Portion', servings: 3, meal_date: '2026-03-25' }],
    }

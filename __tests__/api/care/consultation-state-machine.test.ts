...
import type { AppointmentStatus } from "@/lib/consultation/appointment-status";
...
describe("Appointment State Machine — canTransition", () => {
  ...
  describe("Erlaubte Uebergaenge", () => {
    it("proposed → confirmed durch Gegenseite (patient, wenn doctor proposed)", () => {
      expect(canTransition("proposed", "confirmed", "patient", "doctor")).toBe(
        true,
      );
    });
    ...
    it("proposed → counter_proposed durch Gegenseite", () => {
      expect(
        canTransition("proposed", "counter_proposed", "patient", "doctor"),
      ).toBe(true);
    });

    it("proposed → declined durch Gegenseite", () => {
      expect(canTransition("proposed", "declined", "patient", "doctor")).toBe(
        true,
      );
    });
    ...
    it("counter_proposed → confirmed durch Gegenseite", () => {
      expect(
        canTransition("counter_proposed", "confirmed", "doctor", "patient"),
      ).toBe(true);
    });
    ...
    it("confirmed → cancelled durch beide (patient)", () => {
      expect(canTransition("confirmed", "cancelled", "patient", "doctor")).toBe(
        true,
      );
    });

    it("confirmed → cancelled durch beide (doctor)", () => {
      expect(canTransition("confirmed", "cancelled", "doctor", "patient")).toBe(
        true,
      );
    });

    it("confirmed → active durch doctor", () => {
      expect(canTransition("confirmed", "active", "doctor", "patient")).toBe(
        true,
      );
    });

    it("active → completed durch doctor", () => {
      expect(canTransition("active", "completed", "doctor", "patient")).toBe(
        true,
      );
    });
  });
  ...
  describe("Verbotene Uebergaenge", () => {
    it("proposed → confirmed durch Vorschlagenden SELBST verboten", () => {
      expect(canTransition("proposed", "confirmed", "doctor", "doctor")).toBe(
        false,
      );
    });

    it("proposed → confirmed durch Patient der selbst vorgeschlagen hat verboten", () => {
      expect(canTransition("proposed", "confirmed", "patient", "patient")).toBe(
        false,
      );
    });

    it("confirmed → active durch patient verboten (nur doctor)", () => {
      expect(canTransition("confirmed", "active", "patient", "doctor")).toBe(
        false,
      );
    });

    it("active → completed durch patient verboten (nur doctor)", () => {
      expect(canTransition("active", "completed", "patient", "doctor")).toBe(
        false,
      );
    });
    ...
    it("completed → irgendwas: kein Rueckweg moeglich", () => {
      ...
      for (const target of targets) {
        expect(canTransition("completed", target, "doctor", "patient")).toBe(
          false,
        );
        expect(canTransition("completed", target, "patient", "doctor")).toBe(
          false,
        );
      }
    });
    ...
    it("cancelled → irgendwas: kein Rueckweg moeglich", () => {
      ...
      for (const target of targets) {
        expect(canTransition("cancelled", target, "doctor", "patient")).toBe(
          false,
        );
        expect(canTransition("cancelled", target, "patient", "doctor")).toBe(
          false,
        );
      }
    });
    ...
    it("declined → irgendwas: kein Rueckweg moeglich", () => {
      ...
      for (const target of targets) {
        expect(canTransition("declined", target, "doctor", "patient")).toBe(
          false,
        );
        expect(canTransition("declined", target, "patient", "doctor")).toBe(
          false,
        );
      }
    });
    ...
    it("proposed → active direkt verboten (muss ueber confirmed)", () => {
      expect(canTransition("proposed", "active", "doctor", "patient")).toBe(
        false,
      );
    });

    it("proposed → completed direkt verboten", () => {
      expect(canTransition("proposed", "completed", "doctor", "patient")).toBe(
        false,
      );
    });
  });
  ...
  describe("getAvailableActions", () => {
    ...
    it("Keine Aktionen bei completed", () => {
      expect(
        getAvailableActions("completed", "doctor", "patient"),
      ).toHaveLength(0);
      expect(
        getAvailableActions("completed", "patient", "doctor"),
      ).toHaveLength(0);
    });
...
describe("Consultation confirm Seiteneffekte", () => {
  ...
  async function setupAndPatch(
          ...
          return {
            select: vi.fn().mockReturnValue({
              eq: vi.fn().mockReturnValue({
                single: vi
                  .fn()
                  .mockResolvedValue({ data: slotData, error: null }),
              }),
            }),
            update: mockUpdate,
          };
    ...
    const req = new NextRequest(
      "http://localhost/api/care/consultations/slot-1",
      {
        method: "PATCH",

// Nachbar.io — RSS-Feed-Aggregator Service
// Liest RSS-Feeds von konfigurierten Quellen und importiert
// relevante Artikel in die news_items-Tabelle.
// DSGVO: NUR oeffentliche Nachrichtentexte, KEINE personenbezogenen Daten.
...
export interface RssAggregatorResult {
  message: string;
...
/**
 * RSS-Feed-Aggregator: Holt Artikel aus konfigurierten RSS-Quellen,
 * filtert nach Quartiers-Relevanz, erstellt optional KI-Zusammenfassungen
 * und speichert neue Artikel in der Datenbank.
 */
export async function aggregateRssFeeds(
  supabase: SupabaseClient,
): Promise<RssAggregatorResult> {
...
Format: {"summary": "...", "relevance_score": 0, "category": "infrastructure|events|administration|weather|waste|other"}`,
      ...
      sourceResult.errors.push(
        `Fetch-Fehler: ${err instanceof Error ? err.message : "unbekannt"}`,
      );
    }

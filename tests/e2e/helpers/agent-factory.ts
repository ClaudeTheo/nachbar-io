...
export async function createAgent(
  ...
  if (!credentials) {
    throw new Error(`Unbekannter Agent: ${agentId}. Verfuegbar: ${Object.keys(TEST_AGENTS).join(", ")}`);
  }
  ...
  const context = await browser.newContext({
    baseURL: options?.baseURL || process.env.E2E_BASE_URL || "http://localhost:3000",
    viewport,
    locale: "de-DE",
...
export async function loginAgent(agent: TestAgent): Promise<void> {
  ...
  console.log(`${prefix} Login als ${credentials.displayName} (${credentials.email})`);
...
export async function registerAgent(agent: TestAgent): Promise<void> {
  ...
  await page.getByLabel("Einladungscode").waitFor({ state: "visible", timeout: TIMEOUTS.elementVisible });
  ...
  await page.getByLabel("Anzeigename").waitFor({ state: "visible", timeout: TIMEOUTS.elementVisible });
  await page.getByLabel("Anzeigename").fill(credentials.displayName);
  await page.getByRole("button", { name: "Weiter" }).click();

  // Schritt 4: UI-Modus
  await page.getByText("Aktiver Modus").waitFor({ state: "visible", timeout: TIMEOUTS.elementVisible });
  if (credentials.uiMode === "senior") {

...
import { ConsultationSlotCard } from "@/modules/care/components/appointments/ConsultationSlotCard";
...
export default function ConsultationsPage() {
  ...
  const openSlots = slots.filter(
    (s) => s.status === "proposed" || s.status === "counter_proposed",
  );
  const confirmedSlots = slots.filter(
  ...
  const communitySlots = slots.filter(
    (s) =>
  ...
  return (
    <div className="space-y-6 pb-24">
      <PageHeader title="Meine Termine" backHref="/care" />
      ...
      {activeTab === "confirmed" && communitySlots.length > 0 && (
        <section>
          <h2 className="text-lg font-semibold text-anthrazit mb-3">

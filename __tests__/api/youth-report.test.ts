...
describe('POST /api/youth/report', () => {
  ...
  it('lehnt ab wenn kein authentifizierter User', async () => {
    ...
    const res = await POST(createRequest({ target_type: 'task', target_id: 'abc', reason: 'spam' }));
  ...
  it('lehnt ungueltige target_type ab', async () => {
    const res = await POST(createRequest({ target_type: 'invalid', target_id: 'abc', reason: 'spam' }));
    expect(res.status).toBe(400);
  });
});

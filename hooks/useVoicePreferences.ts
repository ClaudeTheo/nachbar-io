...
import type { VoicePreferences } from '@/components/companion/VoiceSettings'
...
export function useVoicePreferences() {
  const { user } = useAuth()
  const [preferences, setPreferences] = useState<VoicePreferences>(VOICE_DEFAULTS)
    ...
    async function load() {
      ...
      try {
        ...
        if (data?.voice_preferences) {
          ...
          setPreferences({
            voice: (prefs.voice === 'onyx' ? 'onyx' : 'nova') as VoicePreferences['voice'],
            speed: typeof prefs.speed === 'number' ? prefs.speed : 1.0,
            formality: (prefs.formality === 'informal' ? 'informal' : 'formal') as VoicePreferences['formality'],
          })
        }
      } catch (err) {
  ...
  const updatePreferences = useCallback(async (newPrefs: VoicePreferences) => {

...
test.describe('X8: Arzt lehnt Termin ab → Bewohner sieht Absage', () => {
  ...
  test('x8b: Arzt navigiert zur Terminliste und lehnt ab', async ({ arztPage }) => {
  ...
  test('x8c: Bewohner sieht klare Absage-Benachrichtigung', async ({ residentPage }) => {
    ...
    // Auf Absage-Nachricht warten (Realtime oder beim Laden der Seite)
    await waitForRealtimeUI(
      residentPage.page,
      async () => {
        // Absage-Text muss eindeutig sichtbar sein — Bewohner darf nicht im Unklaren bleiben
        await expect(
          residentPage.page.getByText(/abgesagt|abgelehnt|cancelled|absage|leider/i),
        ).toBeVisible();
      },
      { timeout: 20_000 },
    );
